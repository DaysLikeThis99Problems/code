...
//ExchangeRate-API to get APIkey
  ...
  fetch(`https://v6.exchangerate-api.com/v6/${apiKey}/latest/${base}`)
    ...
    .then((data) => {
      ...
      for (const [currency, rate] of Object.entries(data.conversion_rates)) {
        if (currency !== base) {
          ratesHtml += `<li></span>${currency}: ${rate.toFixed(4)}</li>`;
        }
      }
      ratesHtml += "</ul>";
...
//Low to high button
getRatesBtn1=document.getElementById('getRatesBtn1');

getRatesBtn.addEventListener("click", () => {
        getRatesBtn1.style.display = "flex";
  });


getRatesBtn1.addEventListener("click", () => {
    const base = baseCurrency.value;
    ...
    fetch(`https://v6.exchangerate-api.com/v6/${apiKey}/latest/${base}`)
      ...
      .then((data) => {
        ...
        // Sort ratesArray from low to high
        ratesArray.sort((a, b) => a.rate - b.rate);
  
        let ratesHtml = "<h3>Exchange Rates</h3><ul>";
        ratesArray.forEach(({ currency, rate }) => {
          ratesHtml += `<li></span>${currency}: ${rate.toFixed(4)}</li>`;
        });
        ratesHtml += "</ul>";
        exchangeRates.innerHTML = ratesHtml;
