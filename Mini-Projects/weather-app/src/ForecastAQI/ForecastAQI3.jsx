...
function AirQuality({ weather}) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const url='http://api.weatherapi.com/v1/forecast.json?key=cbb3dd09eb154a7bac5202523251401&q='+data.location.name+'&days=3&aqi=yes&alerts=no';

      try {
        const response = await axios.get(url);
        setAQ({MaxTemp:response.data.forecast.forecastday[2].day.maxtemp_c,
            MinTemp:response.data.forecast.forecastday[2].day.mintemp_c,
            AvgTemp:response.data.forecast.forecastday[2].day.avgtemp_c,
            humidity:response.data.forecast.forecastday[2].day.avghumidity,
            maxWind:response.data.forecast.forecastday[2].day.maxwind_kph,
